...
async function main() {
    ...
    for (let i = 0; i < 80; i++) {
        ...
        poDate.setMonth(currentDate.getMonth() - (i % 36));
        ...
        const itemsData = selectedProducts.map(p => {
            const qty = (Math.floor(Math.random() * 7) + 5) * 20;
    ...
    for (let i = 0; i < 100; i++) {
        ...
        orderDate.setMonth(currentDate.getMonth() - (i % 36));
        ...
        let subtotal = 0;
        const itemsData = selectedProducts.map(p => {
            const qty = (Math.floor(Math.random() * 4) + 1) * 15;
